 ...
 *
 * @see {@link StrategyState}
 * @throws {Error} fails if given an invalid strategy instance
 *
 * @param {StrategyState} state - state to validate
 */
...
const validateStrategy = (state = {}) => {
  ...
  if (!_isObject(state)) {
    ...
    } else if (!_isFinite(makerFee) || !_isFinite(takerFee) || makerFee < 0 || takerFee <= 0) {
    ...
    } else if (!_isString(candlePrice) || _isEmpty(candlePrice) || !_includes(VALID_CANDLE_PRICES, candlePrice)) {
      err = `Candle price must be one of ${VALID_CANDLE_PRICES.join(',')} if given`;
