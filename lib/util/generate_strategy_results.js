...
module.exports = (perfManager, strategyState = {}, openPosition = null) => {
  ...
  const positionsPnls = closedPositions.map(p => calcRealizedPositionPnl(p))
  const gains = positionsPnls.filter(pnl => pnl.isPositive())
  const losses = positionsPnls.filter(pnl => pnl.isNegative())
  ...
  const feeCosts = strategyTrades.reduce(
    (total, trade) => trade.fees ? total.plus(trade.fees.cost) : total,
    new BigNumber(0)
  )
