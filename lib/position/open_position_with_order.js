...
const savePositionModel = require('./save_position_model')
...
module.exports = async (state = {}, orderParams = {}) => {
  const { backtesting, onPositionOpen, onOrderFill } = state
  const { mts, tag, amount, stop, target } = orderParams
  ...
  if (!backtesting) {
    ...
    const positionModel = await savePositionModel(nextState, positionData)
