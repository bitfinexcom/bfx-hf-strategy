...
const openPositionWithOrder = async (state = {}, orderParams = {}) => {
  ...
  // if derivative, use max leverage if true else calculate
    const { useMaxLeverage, maxLeverage, leverage } = state
    if (useMaxLeverage) {
      orderParams.lev = maxLeverage
    } else if (backtesting) {
      // skip leverage calculation when backtesting, set user leverage input
      orderParams.lev = leverage
    } else {
      orderParams.lev = calculateLeverage({ ...state, price, amount })
    }
  }
