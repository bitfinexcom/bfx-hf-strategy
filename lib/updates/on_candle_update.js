...
module.exports = async (state = {}, candle = {}) => {
  ...
  if (_isEmpty(candleMarketData)) {
    throw new Error('received candle update, but no candles known')
  }

  // Each live/backtest event message is processed sequentially, and candle
  // updates can only be received for the previous candle, so it is safe to
  // directly update the dataset.
  ...
  if (!state.indicatorData[ikey]) {
    state.indicatorData[ikey] = []
  }

  state.indicatorData[ikey].push(iv)
